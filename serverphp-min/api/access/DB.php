...
class Connect
{
  ...
  private $username = "4008177_bds";
  private $password = "BrH)MQQx8:u}aRY6";
  private $dbname = "4008177_bds";
  private $mysqli; */
  ...
  private $mysqli;
  /* 
  db_name: u701203596_vngate
  db_username: u701203596_vngate
  pass: Nguyen12@
  db_host: localhost
  dbname: u701203596_vngate
  */
...
class DB
{
  ...
  public function select($limitSqlX, $pageNumberX)
  {
    # code...
    //return array("trave" => "ok");
    $pageNumberSql = $pageNumberX;
    $limitSql = $limitSqlX;
    $offsetSql = $pageNumberSql * $limitSql;

    $kqArr = [];
    $product = array();
    $sql = "SELECT * FROM bds LIMIT $limitSql OFFSET $offsetSql;";
    $result = $this->conn->query($sql);
    ...
    if ($result->num_rows > 0) {
      ...
      return array("error" => "loi select", "sqlStr" => $sql);
    }
    return $kqArr;
  }
}
