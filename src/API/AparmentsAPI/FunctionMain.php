...
class  FunctionMain
{
  ...
  */
  ...
  public function getHouse($searchTextRaw)
  {
    //select * from DungDB_DoanhSoHangNgay where ngayThang between '2023-7-01' and '2023-7-03' and mSNV like 'NV001'; 
    ...
    WHERE productID LIKE '%$searchText%' 
    OR  productLocation LIKE '%$searchText%'
    OR  productPrice LIKE '%$searchText%' 
    OR  productStyle LIKE '%$searchText%' 
    OR  productType LIKE '%$searchText%' 
    OR  productDesription LIKE '%$searchText%' 
    OR  productAvailable LIKE '%$searchText%' 
    OR  productUpdate LIKE '%$searchText%' 
    OR  ownerPhone LIKE '%$searchText%' 
    OR  ownerGroup LIKE '%$searchText%' 
    ORDER BY productAvaiLable ASC"; //ASC | DESC
    $this->connectDB();
    if ($this->conn1) {
      $query = mysqli_query($this->conn1, $sql);
      if ($query) {
