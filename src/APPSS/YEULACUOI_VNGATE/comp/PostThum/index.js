...
export default function PostThum(props) {
  ...
  info = {
    code: ptProps.data.code,
    name: ptProps.data.namex,
    sex: "Nữ",
    birthday: "2000-01-12",
    age: 24,
    verify: "Đã xác minh",
  ...
  const photoMain = `https://vngate.top/MEDIA/HINH_KETDOI_VNGATE/${code}/Photos/t.jpg`;
  //--------pop up-----------
  const PopUp3 = (props) => {
    let data2 = {
      typeBds: "Nha Pho",
  ...
  return (
    <>
      ...
      {showModal && <PostModal />}
      <div className="col-12">
        <div className={cx("PostThum")} style={stylePostHome}>
          <div className={cx("header")}></div>
          <div className={cx("body")}></div>
          <div className={cx("bottom")}>
            <div className={cx("info")}>
              <div>
                ID: {info.code} <span>{info.verify}</span>
              </div>
              <div>
                {info.name} <span>{info.age}t</span>
              </div>
              <div>
                {info.dist} - {info.city}
              </div>
